...
def api_production_sfml_generate(payload: dict[str, Any] = Body(default={})):  # noqa: B008
    ...
    try:
        ...
        instructions = (
            ...
            '1) Output ONLY valid SFML v1 plain text. No markdown. No code fences. No explanations. No extra lines before/after SFML.\n'
            '2) The first non-empty line MUST be: # SFML v1\n'
            '3) Use SFML v1 structure exactly:\n'
            '   - cast: block first\n'
            '   - then one or more scene blocks\n'
            '   - inside scenes, use speaker blocks and optional PAUSE lines\n'
            '4) Indentation must be exact:\n'
            '   - cast entries: two spaces\n'
            '   - scene body lines: two spaces\n'
            '   - bullet lines under a speaker: four spaces, format "    - ..."\n'
            '5) Names in speaker blocks MUST exist in the cast map. If the story mentions a speaker not in casting_map, do NOT invent a new cast entry: instead fold that text into Narrator.\n'
            '6) Delivery tags are optional and ONLY in this form at the start of a bullet:\n'
            '   - {delivery=neutral|calm|urgent|dramatic|shout}\n'
            '7) PAUSE lines MUST be exactly: "  PAUSE: <seconds>" where seconds is a decimal (example 0.20). Keep pauses mostly between 0.10 and 2.00.\n'
            '8) Coverage MUST include the full story in order. Do not summarize. Do not add new plot events. You may lightly split/merge paragraphs into scenes for pacing.\n\n'
            'QUALITY TARGET\n'
            'Think like a premium audiobook narrator + TV drama cutscene. Identify dialogue accurately, keep flow natural, use speaker blocks for continuity, and use pauses for pacing. Use delivery tags sparingly when they clearly improve performance.\n\n'
            'INPUTS YOU WILL RECEIVE\n'
            'You will receive:\n'
            '- SFML_SPEC: the authoritative SFML v1 spec\n'
            '- JSON with:\n'
            '  - story.story_md (the full story in markdown)\n'
            '  - casting_map (speaker name -> voice_id), including Narrator\n\n'
            'WORK STEPS (DO SILENTLY, DO NOT OUTPUT THESE STEPS)\n'
            'A) Read casting_map. Build the cast block from it (include all entries exactly as provided).\n'
            'B) Parse story_md. Detect dialogue vs narration. Attribute dialogue to known speakers only.\n'
            'C) Create scenes (scene-1, scene-2, ...) when setting/time/beat shifts. Titles are short, optional.\n'
            'D) Write SFML with correct indentation and bullet formatting.\n\n'
            'FINAL SELF-CHECK (DO SILENTLY)\n'
            'Before you output, verify:\n'
            '- Starts with "# SFML v1"\n'
            '- Exactly one cast block at top\n'
            '- Every scene line starts with "scene scene-"\n'
            '- All speaker blocks are in cast\n'
            '- No markdown fences/backticks anywhere\n'
            '- No commentary outside SFML\n'
            'If any check fails, fix it and re-check.\n\n'
            'NOW OUTPUT THE FINAL SFML v1 FILE.\n'
        )
