...
def api_production_sfml_generate(payload: dict[str, Any] = Body(default={})):  # noqa: B008
    ...
    try:
        ...
        instructions = (
            ...
            '   - Every content line = exactly four spaces + "- " + text\n'
            '   - No inline speaker text\n'
            '   - No bare text lines\n'
            ...
            'TEXT FIDELITY RULE (CRITICAL)\n'
            '- Preserve the original wording exactly.\n'
            '- Do NOT rewrite, paraphrase, summarize, or expand.\n'
            '- Do NOT convert third-person narration into first-person dialogue.\n'
            '- Do NOT change grammatical perspective.\n'
            '- Only restructure into speaker blocks and insert optional pauses.\n'
            '- Only explicit dialogue and explicit italicized internal thoughts may be assigned to characters.\n\n'
            'SPEAKER ATTRIBUTION\n'
            '- Allowed speakers are ONLY exact keys in casting_map.\n'
            '- Normalize minor punctuation variants (e.g., "Maris!", "Ocean?") to match cast keys.\n'
            '- Only assign dialogue to a character when the story clearly indicates that character is speaking.\n'
            '- Standalone italicized internal thoughts (*...*) belong to the active POV character.\n'
            '- If speaker is unclear, assign to Narrator.\n'
            '- Never invent speakers.\n'
            '- Never drop dialogue.\n\n'
            'SPEAKER BLOCK CONSOLIDATION\n'
            '- Do NOT create consecutive blocks for the same speaker within a scene.\n'
            '- Merge adjacent lines from the same speaker.\n'
            '- Do NOT fragment narration into unnecessary micro-blocks.\n\n'
            'SCENE CONTROL\n'
            '- Create a new scene only when there is a meaningful setting, time, or narrative shift.\n'
            '- Avoid splitting into many short scenes.\n\n'
            'PACING\n'
            '- Use PAUSE intentionally for audiobook cadence.\n'
            '- Insert short pauses (0.20–0.40) after emotional beats, standalone short lines, or rhetorical questions.\n'
            '- Insert slightly longer pauses (0.40–0.80) around major turning points.\n'
            '- Avoid flat uninterrupted narration.\n'
            '- Do not overuse pauses.\n\n'
            'QUALITY TARGET\n'
            'Think like a premium audiobook narrator + TV drama cutscene.\n'
            'Maintain emotional continuity while preserving the exact original text.\n\n'
            'INPUTS\n'
            'You will receive:\n'
            '- JSON with:\n'
            '  - story.story_md\n'
            '  - casting_map (including Narrator)\n\n'
            'WORK STEPS (DO SILENTLY)\n'
            'A) Build cast block exactly from casting_map.\n'
            'B) Parse story without altering wording.\n'
            'C) Attribute dialogue strictly.\n'
            'D) Consolidate speaker blocks.\n'
            'E) Output valid SFML.\n\n'
            'FINAL CHECK (DO SILENTLY)\n'
            '- "# SFML v1" exists\n'
            '- "cast:" exists and includes all casting_map entries\n'
            '- No wording changes from original text\n'
            '- No third-person converted to first-person\n'
            '- No consecutive duplicate speaker blocks\n'
            '- All bullets begin with exactly four spaces + "- "\n'
