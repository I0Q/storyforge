...
def api_production_sfml_generate(payload: dict[str, Any] = Body(default={})):  # noqa: B008
    ...
    try:
        ...
        instructions = (
            ...
            '  <SpeakerA>:\n'
            '    - Line\n'
            '    - {delivery=calm} Line with delivery tag\n'
            '  PAUSE: 0.40\n'
            '  <SpeakerB>:\n'
            '    - Line\n'
            '    - Line\n\n'
            'HARD RULES\n'
            '1) First non-empty line must be "# SFML v1".\n'
            '2) The next non-empty line must be "cast:".\n'
            '3) Cast must include EVERY entry from casting_map exactly. No renames. No additions.\n'
            '4) Every scene must be: scene scene-<number> "<Title>" (title required).\n'
            '5) Inside scenes:\n'
            '   - Speaker label is exactly two spaces + "<Name>:"\n'
            '   - Every content line is exactly four spaces + "- " + text\n'
            '   - No other content lines are allowed (no inline "Narrator: text", no bare text).\n'
            '6) PAUSE is optional. If used: exactly two spaces + "PAUSE: <decimal>".\n'
            '7) Delivery is optional. If used, it must be the first thing in a bullet:\n'
            '   {delivery=neutral|calm|urgent|dramatic|shout}\n\n'
            'SPEAKER MAPPING\n'
            '- Allowed speakers are ONLY the exact keys in casting_map.\n'
            '- Normalize variants (e.g., "Maris!", "Ocean?", "the ocean") to the correct cast key.\n'
            '- Never invent speakers.\n'
            '- Never drop dialogue.\n\n'
            'INTERNAL THOUGHT RULE\n'
            '- Standalone italicized lines (*...*) represent internal thoughts.\n'
            '- Attribute them to the active POV character in the surrounding narration.\n'
            '- Do NOT default italic thoughts to Narrator when the POV character is clear.\n\n'
            'SPEAKER BLOCK CONSOLIDATION\n'
            '- Do NOT create consecutive speaker blocks for the same character within a scene.\n'
            '- Merge adjacent lines from the same speaker into a single block.\n'
            '- Avoid unnecessary one-line speaker blocks.\n\n'
            'SCENE CONTROL\n'
            '- Create a new scene only when there is a significant setting, time, or narrative shift.\n'
            '- Avoid many short scenes.\n\n'
            'QUALITY TARGET\n'
            'Think like a premium audiobook narrator + TV drama cutscene.\n'
            '- Maintain natural flow and emotional continuity.\n'
            '- Use speaker blocks to preserve conversational rhythm.\n'
            '- Use PAUSE intentionally to create cadence.\n'
            '- Insert short pauses (0.20–0.40) after emotional beats, short standalone lines, and rhetorical questions.\n'
            '- Insert slightly longer pauses (0.40–0.80) around turning points.\n'
            '- Avoid flat uninterrupted narration.\n'
            '- Use delivery tags sparingly when they enhance performance.\n\n'
            'INPUTS YOU WILL RECEIVE\n'
            'You will receive:\n'
            '- JSON with:\n'
            '  - story.story_md (the full story in markdown)\n'
            '  - casting_map (speaker name -> voice_id), including Narrator\n\n'
            'WORK STEPS (DO SILENTLY)\n'
            'A) Build the cast block from casting_map exactly.\n'
            'B) Parse story_md and detect narration vs dialogue.\n'
            'C) Attribute dialogue using normalization + internal thought rules.\n'
            'D) Create scenes only at meaningful shifts.\n'
            'E) Output clean SFML with correct formatting and consolidated speaker blocks.\n\n'
            'FINAL CHECK (DO SILENTLY)\n'
            '- "cast:" exists and includes all casting_map entries\n'
            '- All speaker labels exactly match casting_map keys\n'
            '- No consecutive duplicate speaker blocks\n'
            '- All bullets start with exactly "    - "\n'
            '- No bullets start with "  - "\n'
            '- Every scene has a quoted title\n'
            '- Output contains nothing outside SFML\n\n'
