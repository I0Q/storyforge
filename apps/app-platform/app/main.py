...
def api_production_sfml_generate(payload: dict[str, Any] = Body(default={})):  # noqa: B008
    ...
    try:
        ...
        instructions = (
            'You are an SFML v1 compiler.\n\n'
            'OUTPUT REQUIREMENTS (MUST FOLLOW)\n'
            '1) Output ONLY valid SFML v1 plain text. No markdown. No code fences. No explanations. No extra lines before or after SFML.\n'
            '2) The first non-empty line MUST be: # SFML v1\n'
            '3) Use SFML v1 structure exactly:\n'
            '   - cast: block first\n'
            '   - then one or more scene blocks\n'
            '4) Every scene MUST include BOTH:\n'
            '   - scene id\n'
            '   - quoted scene title (required, never omit)\n\n'
            'STRUCTURE RULES\n'
            '- Cast block first.\n'
            '- Then scenes formatted as:\n'
            '  scene scene-<number> "<Short Descriptive Title>"\n\n'
            'INDENTATION RULES (STRICT)\n'
            '- Cast entries: two spaces\n'
            '- Scene declaration: no indentation\n'
            '- Speaker label inside scene: exactly two spaces + "<Name>:"\n'
            '- Every spoken or narrated line MUST be a bullet line:\n'
            '  exactly four spaces + "- " + text\n'
            '- No raw text allowed inside scenes.\n'
            '- No inline speaker text (never: "Narrator: sentence").\n'
            '- Every speaker block must contain at least one bullet line.\n\n'
            'PAUSE RULE\n'
            '- Format exactly: two spaces + PAUSE: <decimal>\n'
            '- Example:  PAUSE: 0.40\n'
            '- Use sparingly.\n\n'
            'SPEAKER DETECTION RULES (CRITICAL)\n'
            'You will receive:\n'
            '- SFML_SPEC\n'
            '- JSON:\n'
            '  - story.story_md\n'
            ...
            '1) If a line begins with a known name followed by ":" or dash, assign to that speaker.\n'
            '2) If dialogue appears in quotes and the paragraph attributes it ("Mary said", "says Mary", "Mary asked"), assign to that speaker.\n'
            '3) If a character is referenced by epithet or title that clearly maps to a cast member, treat it as that speaker.\n'
            '4) NEVER drop dialogue. If uncertain, assign to Narrator.\n'
            '5) Never invent speakers not in casting_map.\n'
            '6) All speaker labels used must exist in cast.\n\n'
            'CAST RULES\n'
            '- Include EVERY entry from casting_map in the cast block.\n'
            '- Do not remove unused characters.\n'
            '- Do not rename characters.\n'
            '- Do not invent characters.\n\n'
            'SCENE RULES\n'
            '- Create scenes at major beat or setting shifts.\n'
            '- Titles must be 3 to 8 words.\n'
            '- Titles must not contain punctuation except apostrophes.\n\n'
            'DELIVERY TAGS (OPTIONAL)\n'
            'If used, must appear at the start of a bullet only:\n'
            '{delivery=neutral|calm|urgent|dramatic|shout}\n\n'
            'VALIDATION (DO SILENTLY BEFORE OUTPUT)\n'
            '- Starts with "# SFML v1"\n'
            '- Cast includes all casting_map entries\n'
            '- Every scene has id AND quoted title\n'
            '- Every speaker block contains bullet lines with exactly four spaces + "- "\n'
            '- No bare text lines inside scenes\n'
            '- No speaker outside cast\n'
            '- No markdown, no commentary\n\n'
            'If any rule fails, rewrite until it passes.\n\n'
            'NOW OUTPUT THE FINAL SFML v1 FILE ONLY.\n'
        )
